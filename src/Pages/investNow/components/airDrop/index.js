...
import {
  ...
  handleLoan45A,
  handleStakingDailyA,
  handleStakingMonthlyA,
  handleStakingQuarterlyA,
...
const AirDrop = () => {
  ...
  useEffect(() => {
    const abc = async () => {
      const result = await web3.eth.getAccounts((e, r) => {
        setAddress(r[0]);
      });
      contract.methods.MCAS().call((e, r) => {
        setMCAS(r);
      });
      contract.methods.MCASIssued().call((e, r) => {
        setMcasIssued(r);
      });
      contract.methods.ESAS().call((e, r) => {
        setESAS(r);
      });
      contract.methods.ESASIssued().call((e, r) => {
        setESASIssued(r);
      });
      contract.methods.TDS().call((e, r) => {
        setTDS(r);
      });
      contract.methods.TDSIssued().call((e, r) => {
        setTDSIssued(r);
      });
      lotteryContract.methods.BuyTicketsActive().call((e, r) => {
        setlotteryPurcahseActive(r);
      });
      loanContract.methods.BUYTFTActive().call((e, r) => {
        setTFTBuyACtive(r);
      });
      loanContract.methods.stakingActive().call((e, r) => {
        setStakingActive(r);
      });
      loanContract.methods.lstakingActive().call((e, r) => {
        settftLoanActive(r);
      });

      contract.methods.Team().call((e, r) => {
        setTEAM(r);
      });
      contract.methods.MCASDetails(result[0]).call((e, r) => {
        setMcasDetails(r);
      });
      contract.methods.ESASDetails().call((e, r) => {
        setEsasDetails(r);
      });
      contract.methods.teamClaimVested().call((e, r) => {
        setTokenVested(r);
      });
    };
    abc();
  ...
  function handleMCASSubmit() {
    if (csv && scheme) {
      ...
      csv.map((v, i) => {
        myArray.push([
          v[0],
          Number(v[1]) * 100000000,
          Math.floor(currentTime) + Number(v[2]) * 60 * 60 * 24,
        ]);
      });
    ...
    if (quantity && days) {
      ...
      contract.methods
        .setESAS(
          scheme,
          quantity * 100000000,
          Math.floor(currentTime) + days * 60 * 60 * 24
        )
        .send({ from: address })
        .on("confirmation", (e, r) => {
          setDays("");
    ...
    <div className="airDrop-main-wrapper">
      {Admin == address ? (
        <div className="AdminBlock">
          ...
          <p>
            Mkt and Comm AirDrop{"  "}
            <span>{`${mcasIssued / 100000000} / ${mcas / 100000000}`}</span>
          </p>
          <p>
            Strategic Adv AirDrop{"  "}
            <span>{`${esasIssued / 100000000} / ${esas / 100000000}`}</span>
          </p>
          <p>
            Team AirDrop{"  "}
            <span>{`${tdsIssued / 100000000} / ${tds / 100000000}`}</span>
          </p>
          ...
          <div style={{ border: "1px solid black" }}>
            ...
            ></input>
            <div className="Recipients_info_2Qot1" data-v-404281ee="">
              <input
          ...
          <button
            onClick={() => {
              contract.methods.approve().send({ from: address });
            }}
          >
            Approve tokens
          </button>
          ...
          <button onClick={ActivateLotteryPurchase}>
            {LotteryPurchaseActive
              ? "Inactive Lottery Purchase"
              : "Activate lottery purchase"}
          </button>
          ...
          <div>
            <input
              value={stakingDaily}
              onChange={(e) => {
                setStakingDaily(e.target.value);
              }}
              type="value"
            />

            <button onClick={handleStakingDaily}>set staking Daily rate</button>
          </div>
          ...
          <div>
            <input
              ...
              onChange={(e) => {
                setStakingMonthly(e.target.value);
              }}
              type="value"
            />
          ...
          <div>
            <input
              ...
              onChange={(e) => {
                setStakingQuarterly(e.target.value);
              }}
              type="value"
            />
          ...
          <div>
            <input
              value={loan30}
              onChange={(e) => {
                setLoan30(e.target.value);
              }}
              type="value"
            />

            <button onClick={handleLoan30}>set Loan 30 rate</button>
          </div>
          ...
          <div>
            <input
              value={loan45}
              onChange={(e) => {
                setLoan45(e.target.value);
              }}
              type="value"
            />

            <button onClick={handleLoan45}>set Loan 45 rate</button>
          </div>
          ...
          <div>
            <input
              value={changeDisc}
              onChange={(e) => {
                setChangeDisc(e.target.value);
              }}
              type="value"
            />

            <button onClick={changeDiscount}>Change discount %</button>
          </div>
        </div>
      ) : (
        <div>
          <div className="marketing-title">
            Marketing and Community Panel Panel
          </div>
        ...
        {`${(mcasIssued/100000000).toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} /
         ${(mcas/100000000).toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`}</span></p> */}
          <div className="airdrops-box">
            <div className="airdrop-title">Airdrops</div>
            <table>
              <thead className="airdrop-table-header">
                <tr>
              ...
              <tbody>
                {mcasDetails &&
                  mcasDetails.map((val, key) => {
                    var currentTime = new Date().getTime() / 1000;
                    return (
                      <tr key={key}>
                        <td>{val.Scheme}</td>
                        <td>{val.amount / 100000000}</td>
                        {(Number(val.Days) - currentTime) / (60 * 60 * 24) >
                        0 ? (
                          <td>
                            {(
                              (Number(val.Days) - currentTime) /
                              (60 * 60 * 24)
                            ).toFixed(0)}
                          </td>
                        ) : (
