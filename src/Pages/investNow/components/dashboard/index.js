...
const Dashboard = ({ handleBuyAtDiscountClick, handleBuyAtMarketClick }) => {
  ...
  const loanIssued = useSelector((state) => {
    return  ((Number(state.adoptReducer.loanIssued)/1000000000000000000).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
  });
    ...
    <div className="dashboard-wrapper">
      ...
      <div className="stockBoxWrapper">
        <div className="stockBoxWrapper-row">
          <StockBox text="Current Circulating Supply - Million" value={((circulatingSupply/100000000000000).toFixed(0)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} />
          <div className="stock-middle">
            <StockBox text="% off TFT Staked + Deposited" value={tftStakedDeposited} />
          </div>
