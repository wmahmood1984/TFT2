...
const OneFor10X = () => {
  ...
  var filteredInfo =
    _lottCurrDetails &&
    _lottCurrDetails.tx1.filter((item) => item.id == _lottCurrDetails._counter);
  ...
  function BuyLottery() {
    ...
    if (Number(_TFTAllowance) >= 10000000 * array.length) {
      ...
      dispatch(
        LotteryApprove({ quantity: Math.floor(0.1 * array.length * 100000000) })
      );
    }
  ...
  _lottCurrDetails &&
    filteredInfo.map((v, i) => {
      ...
      if (Number(v.number) == 2) {
        _two.push(v);
      }
      if (Number(v.number) == 3) {
        _three.push(v);
      }
      if (Number(v.number) == 4) {
        _four.push(v);
      }
      if (Number(v.number) == 5) {
        _five.push(v);
      }
      if (Number(v.number) == 6) {
        _six.push(v);
      }
      if (Number(v.number) == 7) {
        _seven.push(v);
      }
      if (Number(v.number) == 8) {
        _eight.push(v);
      }
      if (Number(v.number) == 9) {
        _nine.push(v);
      }
    });
  ...
  var NewTime =
    gameDetails && gameDetails.length != 0
      ? Number(gameDetails[gameDetails.length - 1].time) + 120
  ...
  function Status() {
    ...
    if (winnNumber.length == 0) {
      ...
      setwinStatus(
        `Congratulations, your winning numbers is ${[
          ...new Set(winnNumber),
        ].toString()}. For more details plz check your history tab below.`
      );
    }
  }
  ...
  return (
    <div className="theFairWin-main-wrapper">
          ...
          <tr>
            ...
            <td>
              <input
                ...
                onChange={(e) => {
                  setSeven(Number(e.target.value));
                }}
                type="value"
              />
            </td>
          ...
          <tr>
            ...
            <td>
              <input
                ...
                onChange={(e) => {
                  setEight(Number(e.target.value));
                }}
                type="value"
              />
            </td>
      ...
      <div className="theFairWin-section-wrapper">
        ...
        <h5>Draw # {gameCounter}</h5>
        {
          <div>
            {_zero.length > 0 ? (
              <p>
                #0 x {_zero.length} Tickets =&gt; {_zero.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_one.length > 0 ? (
              <p>
                #1 x {_one.length} Tickets =&gt; {_one.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_two.length > 0 ? (
              <p>
                #2 x {_two.length} Tickets =&gt; {_two.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_three.length > 0 ? (
              <p>
                #3 x {_three.length} Tickets =&gt; {_three.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_four.length > 0 ? (
              <p>
                #4 x {_four.length} Tickets =&gt; {_four.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_five.length > 0 ? (
              <p>
                #5 x {_five.length} Tickets =&gt; {_five.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_six.length > 0 ? (
              <p>
                #6 x {_six.length} Tickets =&gt; {_six.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_seven.length > 0 ? (
              <p>
                #7 x {_seven.length} Tickets =&gt; {_seven.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_eight.length > 0 ? (
              <p>
                #8 x {_eight.length} Tickets =&gt; {_eight.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
            {_nine.length > 0 ? (
              <p>
                #9 x {_nine.length} Tickets =&gt; {_nine.length * 0.9} TFT
                Possible Win
              </p>
            ) : null}
          </div>
      ...
      <div className="theFairWin-section-wrapper">
        ...
        <div className="checkNow-btn-wrapper">
          {winStatus ? (
            ...
            <button onClick={Status} className="checkNow-btn">
              <span>CHECK NOW</span>
            </button>
          )}
        </div>
      ...
      <div className="theFairWin-section-wrapper">
        <div className="finished-draws-title">FINISHED DRAWS</div>
        <div className="history-buttons-wrapper">
          <div
            className="history-button"
            style={
              allHistory
                ? { color: "#2F5D8E", borderBottom: "3px solid #2F5D8E" }
                : null
            }
        ...
        {allHistory ? (
          <HistoryTable tab="AllHistory" details={gameDetails && gameDetails} />
        ) : null}
        {yourHistory ? (
          <HistoryTable
            tab="yourHistory"
